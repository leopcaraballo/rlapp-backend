...
public sealed class CallNextCashierCommandHandler
{
    private readonly IEventStore _eventStore;
    private readonly IEventPublisher _eventPublisher;
    private readonly IClock _clock;

    public CallNextCashierCommandHandler(
    ...
    public async Task<(int EventCount, string PatientId)> HandleAsync(
        CallNextCashierCommand command,
        CancellationToken cancellationToken = default)
    {
        ...
        var request = new CallNextCashierRequest
        {
            CalledAt = _clock.UtcNow,
            Metadata = metadata,
            CashierDeskId = command.CashierDeskId
        };

        var patientId = queue.CallNextAtCashier(request);
